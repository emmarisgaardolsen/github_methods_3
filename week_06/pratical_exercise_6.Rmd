...
a_list = [1, 2] # initiate a list (the square brackets) with the integers 1 and 2
  # a little like c() function in R. 
  # doesn't need to be the same time, could be numbers, letters, etc.
...
print(new_list[0:3])  # slicing, choosing from the 0th up to the 3rd, not including 3?
                      # you can slice backwards using negative variables
                      
# for, if, else, while: you don't have to use brackets, but you HAVE to use indt properly.
  # as soon as you go out of your indentation you go out of the loop 
...
this_is_true = True # logical values, only first letter capitalized
this_is_false = False
...
# import: importing packages (similar to library)
# difference from R: you specify package AND function. e.g. "numpy.arrange"
...
import numpy as np # you can import them with another name than its default
  # the one above allow you to name it differently!! 
  
print(np.cos(np.pi))

from numpy import pi, arange # or you can import specific methods from a package
...
x = np.arange(10) # 0 to 9. our x values
y = 2 * x # our y values. all values from 0 to 9, each of them multiplied by 2
y = y.astype(float) # adding decimals?
n_samples = len(y)
y += np.random.normal(loc=0, scale=1, size=n_samples) # kommatal med decimaler
  # loc = center of distribution (mean)
  # scale = standard


X = np.zeros(shape=(n_samples, 2)) # matrix with empty entries
X[:, 0] = x ** 0 # everything in first col is "opløftet i 0". anything opløftet i 0 is 1 :)))))
X[:, 1] = x ** 1 # everything in the second col (1 is 2nd because index starts at 0)


# Get OLS coefficient estimates from (X*X)^(-1)X*Y
  # .T transposes a matrix
  # dot(X,Y) is for matrix multiplication

bhat = np.dot(np.linalg.inv(X.T @ X), X.T @ y))
print(bhat)

# y_hat = X * bhat
yhat = X @ bhat # matrix multiplication
print(yhat)


# plot a scatter plot of _x_, _y_ and add a line based on $\hat{y}$ (use `plt.plot` after running `import matplotlib.pyplot as plt`)  
...
plt.figure() 
plt.plot(x, y, 'o', color='pink') #create basic scatterplot
...
plt.xlabel('x')
plt.title('MY first linear regression plot wehuu')
