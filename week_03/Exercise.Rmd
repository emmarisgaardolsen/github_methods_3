...
*trial.type is a character variable but should be classified into a factor in order for them to be modelled later in the analysis. you could expect reactiontime/accuracy to systematically change dependent on specific trial types*
```{r}
df$trial.type <- as.factor(df$trial.type)
```

*pas is a numeric variable. It indicates whether you have seen the stimulus (the variable can take 4 different values)*
```{r}
df$pas <- as.factor(df$pas)
```

*trial is numeric but should be factor*
```{r}
df$trial <- as.factor(df$trial)
```

*target.contrast is the contrast level of the stimulus. it is numeric and should stay numeric*
