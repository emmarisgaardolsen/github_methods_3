...
exp_count4 <- data_count %>% 
  filter(subject == '4' | subject == '3' | subject == '19' | subject == '25')

plot_final <- list()
for (i in 1:length(exp_count4)){
  
  data.count$fitted <- fitted.values(countmodel1)
  plot_temp <- data.count %>% 
    filter(subject == exp_count4[i]) %>% 
    ggplot(aes(pas, fitted, fill = pas)) +
    geom_bar(stat = "identity") +
    ggtitle(paste(c('Estimated amount of ratings for Subject', exp_count4[i]), collapse=', ' )) + ylim(0,300) +
...
```
   ...
   Yes, task seems to explain performance as single tasks significantly differs from task pair (p < .05). The negative coefficients indicate that an increase in task complexity results in a decrease in the amount of correct answers. 

  ii. add _pas_ as a main effect on top of _task_ - what are the consequences of that?  
  
```{r}
finalmodel2 <- glmer(correct ~ task + pas + (1 | subject), data = exp, family = 'binomial')
```
...
```{r}
finalmodel3 <- glmer(correct ~ pas + (1 | subject), data = exp, family = 'binomial')
```
...
finalmodel4 <- glmer(correct ~ task*pas + (1 | subject), data = exp, family = 'binomial')
...
AIC <- c(AIC(finalmodel1), AIC(finalmodel2), AIC(finalmodel3),AIC(finalmodel4))
resvar <- c(sum(residuals(finalmodel1)^2),sum(residuals(finalmodel2)^2),sum(residuals(finalmodel3)^2),sum(residuals(finalmodel4)^2)) #residual variance
res_sd<- c(sqrt((sum(residuals(finalmodel1))^2/length(residuals(finalmodel1))-2)),sqrt((sum(residuals(countmodel2))^2/length(residuals(countmodel2))-2)),sqrt((sum(residuals(finalmodel3))^2/length(residuals(finalmodel3))-2)),sqrt((sum(residuals(finalmodel3))^2/length(residuals(finalmodel3))-2)),sqrt((sum(residuals(finalmodel4))^2/length(residuals(finalmodel4))-2))) # residual standard deviation 

as_tibble(cbind(rownames, AIC, resvar, res_sd))

```

The last the models almost has the same AIC values, residual variance, and residual standard deviation, suggesting thatm, as compared to *task*, *pas* is a better predictor. Nonetheless, the model with the interaction term would be easier to justify by theory, as one would expect perceptual awareness rating to interact with the complexity of task (for reasons also elaborated earlier)
