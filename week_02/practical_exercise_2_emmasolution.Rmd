...
The attitude variable is a binary variable taking two possible values, i.e "inf" (which corresponds to "informal") and "pol" (which corresponds to "formal").The participants were asked to do each scenario in a formal and informal way. Each trial/scenario (n = 14) is either "informal" or "polite", making it 50% "informal" and 50% "polite". For analytic purposes it will be transformed into a factor variable. 
...
*[,5] scenario (i.e. an indicator of which of the 7 different scenarios the participant is in)*
...
# Model matrices
Xinteger <- model.matrix(lm1) # model matrix with scenario as integer
Xfactor <- model.matrix(lm2) # model matrix with scenario as factor

Xinteger
Xfactor
```

*The first model model (lm1) expresses f0mn as dependent on scenario as an integer. When the "scenario" variable is integer, it is treated as numeric by R and we are modelling one intercept and one slope. This means we would interpret the slope in the summary of the model as follows: per increment of scenario the pitch will go down by 6.886 hz. This is a faulty interpretation, as the 2x2 matrix shows that we would then interpret scenario 7, as being 7 times more than scenario 1, and this is not the case.*

*The matrix made with scenario as an integer is a 2x2 design matrix that does not treat each scenario separately. If running a model based upon this, we would run into issues and throw away possible useful information.* 

*Model 2 (lm2) expresses f0mn as dependent on scenario encoded as a factor. Coding scenario as a factor is more fitting. When the "scenario" variable is factor, it is treated categorically by R,meaning that we are modelling one intercept and 6 different slope, one for each scenario. This enables us to interpret the output of the model 2 correctly as follows; going from scenario 1 to scenario 2 the pitch increases by 62.40 hz, going from scenario 1 to scenario 3 the pitch increases by 35.35 hz etc. The model matrix of model 2 shows multiple columns (one for each scenario), meaning that we can look at each scenario individually, and thereby investigate their meaning properly.*
...
The main difference between each subject is the pitch of voice in hertz. The subplots show that the male subjects tend to have lower frequency (i.e. lower pitch of voice), and the female subjects tend to have higher pitch of voice (comparing the y-axis position of the data points of subjects starting with 'F' and subjects starting with 'M'). Likewise, the subplots demonstrate how there are individual differences within pitch of voice (some subjects change pitch more depending on attitude, for instance), meaning that we must account for this by including subjects as a random effect when modeling the data. For most subjects the attitudes don't differ uch across scenarios. There are, however, exceptions (F4, F5, F6, M4). 
...
The short answer is that it doesn't take individual differences in pitch into account. This will be described more below. 
