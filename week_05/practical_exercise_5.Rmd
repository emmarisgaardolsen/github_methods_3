...
```{r}
library(tidyverse,dplyr, data.table, vroom, ggplot2, readbulk, lme4, rstanarm, MuMIn, lmerTest, lme4)
...
*_target.contrast_: in experiment 1, the experimenters didn't adjust the target contrast to match the threshold of each individual. Therefore, the value of this variable is 0.1 for all subjects. They did, however, add staircasing trials in experiment 2, which is why we had different values of target.contrast for each subject in part 1 of this assignment (which reflected Experiment 2 from the paper)*
...
m_partial <- glmer(correct~target.frames+(1|subject), family=binomial(link=logit),data=df) # modeling both an average and each level of the subjects 
